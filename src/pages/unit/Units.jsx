...
const Units = () => {
  ...
  const getUnits = async () => {
    const response = await axios.get(
      `http://localhost:5000/units?search_query=${keyword}&page${page}&limit${limit}`
    );
...
<div className="mt-14 ml-14 mb-10  sm:mt-5">
        <Header category="Halaman" title="Satuan" />
      </div>
      ...
      <div className="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
        ...
        <Link className="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3" to={'/satuan/tambah-satuan'}>
            <Button
              color="white"
        ...
        <div className="table-layout:fixed overflow-y-hidden rounded-2xl shadow-md">
          <Table hoverable={true} className='sm:table-fixed overflow-y-hidden'>
            <Table.Head className="bg-gray-300">
              ...
              <Table.HeadCell className="w-96">Nama Satuan</Table.HeadCell>
